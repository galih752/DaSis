...
include("koneksi.php");
$sql = mysqli_query($koneksi, "SELECT * FROM guru WHERE kode='$_GET[nip]'");
$data = mysqli_fetch_array($sql);
$sql1 = mysqli_query($koneksi, "SELECT * FROM mapel WHERE kode='$_GET[nip]'");
  ...
  <form class="row g-3" action="" method="post">
    ...
    <div class="col-md-6">
      ...
      <input type="text"
        ...
        id="jabatan" name="jabatan" value="<?php echo $data['jabatan'] ?>">
    </div>
    <div class="col-md-3">
      <label for="validationDefault05" class="form-label">Mata Pelajaran</label>
      <select name="mapel" id="mapel"
        style="text-align:center;display: block;width: 66%;padding: 0.375rem 0.75rem;font-size: 1rem;font-weight: 400;line-height: 1.5;color: #212529;background-color: #fff;background-clip: padding-box;border: 1px solid #ced4da;-webkit-appearance: none;-moz-appearance: none;appearance: none;border-radius: 0.375rem;transition: border-color 0.15s ease-in-out, box-shadow 0.15s ease-in-out;">
        <option><?php echo $data1['namamapel'];?></option>
        <?php
        include("koneksi.php");
